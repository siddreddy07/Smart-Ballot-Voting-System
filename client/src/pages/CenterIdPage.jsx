...
export default function CenterIdPage() {
  const { id } = useParams();
  const totalVotes = boothData.reduce((sum, booth) => sum + booth.counted, 0);
    ...
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex items-center space-x-4">
          ...
          <img src="/placeholder.svg" alt="Officer" className="w-16 h-16 rounded-full" />
        </div>
      </div>

      <h2 className="text-2xl font-semibold mb-6">Booth Overview for Center {id}</h2>
      <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-5 mb-8">
        {boothData.map((booth) => (
          <div key={booth.number} className="bg-white rounded-lg shadow p-4">
            <h3 className="font-semibold mb-2">Booth {booth.number}</h3>
      ...
      <div className="bg-white rounded-lg shadow p-4 mb-8">
        <h3 className="text-xl font-semibold mb-2">Total Booths in Center</h3>
        <p className="text-4xl font-bold">{boothData.length}</p>
      </div>

      <div className="bg-white rounded-lg shadow p-4">
        <h3 className="text-xl font-semibold mb-4">Votes Per Booth</h3>
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Booth Name</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Registered Voters</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Votes Counted</th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-gray-200">
              ...
              <tr>
                <td colSpan={2} className="px-6 py-4 whitespace-nowrap font-bold">Total Votes Counted</td>
                <td className="px-6 py-4 whitespace-nowrap font-bold">{totalVotes}</td>
              </tr>
            </tbody>
          </table>
        </div>
